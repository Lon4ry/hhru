...
const schema = z
  .object({
    ...
    salaryFrom: z.coerce.number().min(10000, "Значение должно быть больше 10 000"),
    salaryTo: z.coerce
      .number()
      .min(10000, "Значение должно быть больше 10 000")
      .refine((value, ctx) => value >= ctx.parent.salaryFrom, "Верхняя граница не может быть ниже нижней"),
  })
...
export default function NewVacancyPage() {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      requirements: "5+ лет с React, глубокое знание TypeScript, опыт построения дизайн-систем.",
  ...
  return (
    <div className="grid gap-6">
      <div>
        <h1 className="text-2xl font-semibold text-slate-900">Новая вакансия</h1>
        <p className="text-sm text-slate-500">
          Заполните форму, проверьте значения зарплаты и требования — система покажет валидационные подсказки в реальном
          времени.
        </p>
      </div>
      ...
      <Card className="space-y-4">
        <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600">
          <p>
            <span className="font-semibold text-slate-900">Пример:</span> укажите «от {form.watch("salaryFrom")?.toLocaleString("ru-RU")}
            ₽ до {form.watch("salaryTo")?.toLocaleString("ru-RU")} ₽» — форма проверит, чтобы верхняя граница была выше нижней и
            диапазон не превышал 500 000 ₽.
          </p>
        </div>
        <form className="grid gap-4" onSubmit={form.handleSubmit(onSubmit)}>
          <Input label="Должность" {...form.register("title")} error={form.formState.errors.title?.message} />
          <Textarea
            label="Описание"
          ...
          />
          <Textarea label="Условия" {...form.register("conditions")} error={form.formState.errors.conditions?.message} />
