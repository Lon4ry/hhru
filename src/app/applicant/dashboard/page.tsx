...
const statusLabels: Record<
  string,
  { title: string; variant: "neutral" | "success" | "warning" | "danger" }
> = {
...
export default async function ApplicantDashboardPage() {
  ...
  const { applications, notifications, resume, statusMap } =
    await getApplicantData(userId);
    ...
    <div className="grid gap-6">
      <div className="grid gap-4 md:grid-cols-3">
        <StatsCard
          ...
          value={resume?.desiredPosition ?? "Не указано"}
        />
        <StatsCard
          title="Ожидаемая зарплата"
          value={formatCurrency(resume?.expectedSalary)}
        />
        <StatsCard
      ...
      <Card className="space-y-4">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Лента важных событий, приглашений и обновлений платформы.
          </p>
        </div>
        {notifications.length === 0 ? (
          <EmptyState
            title="Пока нет уведомлений"
            description="Откликайтесь на вакансии, чтобы получать приглашения."
          />
        ) : (
      ...
      <Card className="space-y-4">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Отслеживайте статус заявок и приглашений от работодателей.
          </p>
        </div>
        {applications.length === 0 ? (
          <EmptyState
            title="Вы ещё не откликались"
            description="Перейдите в раздел вакансий и отправьте первый отклик."
          />
        ) : (
              ...
              >
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                  <div>
                    ...
                    <p className="text-sm text-slate-500">
                      {application.vacancy.company?.name ?? "Компания"} •{" "}
                      {application.vacancy.city}
                    </p>
                    <p className="text-sm text-slate-500">
                      {formatCurrency(application.vacancy.salaryFrom)} —{" "}
